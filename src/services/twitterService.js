...
try {
  ...
  logger.warn(
    "jokeRepository could not be loaded (sqlite3 unavailable?): %s",
    e.message,
  );
}
...
async function publishPost(jokeObj) {
  ...
  while (attempt < maxRetries) {
    ...
    try {
      logger.info(
        "Attempt %d of %d to post for joke ID %d.",
        attempt,
        maxRetries,
        jokeObj.id,
      );
      ...
      if (process.env.POST_IMAGE !== "false") {
        try {
          logger.info(
            "Generating dynamic image card for joke ID %d...",
            jokeObj.id,
          );
          const imageBuffer = await imageGenerator.generateJokeCard(jokeObj);

          logger.info("Uploading image card blob...");
          const uploadRes = await bluesky.uploadBlob(imageBuffer, "image/jpeg");
          ...
          logger.error(
            "Failed to generate/upload image card: %s. Falling back to text-only post.",
            imgError.message,
          );
        }
      }
