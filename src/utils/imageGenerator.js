...
try {
  ...
  GlobalFonts.registerFromPath(path.join(ASSETS, 'Sora-Variable.ttf'),   FONT_SORA);
} catch (e) { logger.error('Font load: %s', e.message); }
...
// ── Category → background ────────────────────────────────────
const CATEGORY_BG = {
  'daily life':'daily_life.png','indian parents':'indian_family.png',
  'indian family':'indian_family.png','family':'indian_family.png',
  'office':'office.png','corporate':'office.png',
  'programming':'programming.png','college':'college.png',
  'student':'college.png','relationships':'relationships.png',
  'relationship':'relationships.png','social media':'social_media.png',
  'internet':'social_media.png','money':'money.png','shopping':'money.png',
  'food':'food.png','gaming':'gaming.png',
};
const FALLBACKS = Array.from({ length: 15 }, (_, i) => `bg${i}.png`);

function getBgPath(cat, id) {
  const f = CATEGORY_BG[cat];
  if (f) { const p = path.join(BG_DIR, f); if (fs.existsSync(p)) return p; }
...
async function prepareBg(bgPath) {
  return sharp(bgPath)
    ...
    .blur(8)
    .modulate({ brightness: 0.88, saturation: 0.90 })
...
const IMPACT_WORDS = new Set([
  // emotions & reactions
  'crying','laughing','screaming','dying','dead','shocked','confused',
  'embarrassed','terrified','furious','desperate','panic','anxiety',
  'depressed','excited','obsessed','addicted','exhausted','stressed',
  // strong verbs
  'quit','fired','broke','failed','forgot','lied','cheated','ghosted',
  'blocked','deleted','crashed','exploded','destroyed','survived',
  'regret','blame','admit','pretend','ignore','avoid','escape',
  // punchline nouns
  'wifi','money','sleep','coffee','deadline','meeting','boss','salary',
  'diet','gym','exam','grade','debt','rent','budget','interview',
  'password','update','error','bug','feature','reboot','backup',
  'ex','crush','date','wedding','divorce','baby','mom','dad',
  'doctor','dentist','therapist','lawyer','accountant',
  // intensifiers that land
  'never','always','literally','actually','basically','technically',
  'officially','immediately','suddenly','finally','already','still',
  'worst','best','only','free','fake','real','wrong','right',
  // comedy contrast words
  'but','except','unless','until','meanwhile','suddenly','plot',
  'twist','surprise','secret','truth','lie','reality','dream',
]);

const STOP = new Set([
  'the','and','is','in','of','to','a','an','it','on','at','be','as','by',
  'or','not','so','if','me','my','we','he','she','they','you','i',
  'was','are','were','has','had','have','do','did','does','for','with',
  'that','this','from','your','our','their','its','will','can','may',
  'just','also','then','than','when','what','how','who','which','all',
]);
...
function drawLine(ctx, line, cx, baseY, baseFs, hlSet) {
  ...
  const parts = tokens.map(tok => {
    const clean = tok.trim().toLowerCase().replace(/[^a-z]/gi, '');
...
async function generateJokeCard(jokeObj) {
  ...
  ctx.lineWidth   = 1;
  ctx.beginPath(); ctx.moveTo(W/2 - lw/2 - 28 - 200, TY); ctx.lineTo(W/2 - lw/2 - 28, TY); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(W/2 + lw/2 + 28, TY);       ctx.lineTo(W/2 + lw/2 + 28 + 200, TY); ctx.stroke();
  ctx.restore();
  ...
  const hlPhrases = (jokeObj.highlights && jokeObj.highlights.length)
  ...
  for (const phrase of hlPhrases) {
    for (const w of phrase.split(/\s+/)) hlSet.add(w.replace(/[^a-z]/gi, '').toLowerCase());
  }
  ...
  const totalFW = (ISZ + LG + lkW) + GG + 1 + GG + (ISZ + LG + shW) + GG + 1 + GG + (ISZ + LG + flW);
